...
public class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    public MyHashMap() {
        table = new Node[capacity]; // Ініціалізуємо таблицю з початковою місткістю
    }
    ...
    public int getIndex(K key) {
        ...
        // Повертаємо індекс після обчислення модуля та гарантуємо,
        // що він буде додатнім
    }
    ...
    public void put(K key, V value) {
        ...
        if (size >= threshold) {
            ...
            // Перераховуємо поріг для нової місткості
            ...
            for (int i = 0; i < table.length; i++) { // Перебираємо стару таблицю
                ...
                while (current != null) {
                    // Перебираємо всі вузли на поточному індексі
                    ...
                    current.next = newTable[newHash]; // Встановлюємо новий зв'язок
                    newTable[newHash] = current;
                    // Поміщаємо поточний вузол на новий індекс
        ...
        // Отримуємо вузол, який знаходиться на цьому індексі
        while (current != null) { // Перебираємо всі елементи в ланцюзі
            if ((key == null && current.key == null)
                ...
                return; // Виходимо, бо елемент вже є в мапі
            }
