...
public class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    private void putForNullKey(V value) {
        Entry<K, V> checkKeyNull = checkKey(null);
        if (checkKeyNull == null) {
            Entry<K, V> bucket = table[0];
            table[0] = new Entry(null, value, bucket);
            size++;
        } else {
            checkKeyNull.value = value;
        }
    }
    ...
    public void put(K key, V value) {
        if (size > countThreshold(capacity)) {
            resizeHashMap();
        }
        if (key == null) {
            putForNullKey(value);
        } else {
            Entry<K, V> checkKey = checkKey(key);
            if (checkKey == null) {
    ...
    public V getValue(K key) {
        Entry<K, V> checkKey = checkKey(key);
        return checkKey != null ? checkKey.value : null;
    }
