package core.basesyntax;

public class MyHashMap<K, V> implements MyMap<K, V> {
    private static final int DEFAULT_CAPACITY = 16;
    private static final int GROW_CONSTANT = 2;
    private static final double GROW_INDEX = 0.75;

    private int size;
    private Node<K, V>[] hashMap;

    public MyHashMap() {
        hashMap = new Node[DEFAULT_CAPACITY];
    }

    @Override
    public void put(K key, V value) {
        int hash = getHash(key);
        Node<K, V> newNode = new Node<>(hash, key, value, null);
        int index = getIndex(hash);
        if (hashMap[index] == null) {
            hashMap[index] = newNode;
            size++;
        } else {
            Node<K, V> node = hashMap[index];
            while (node.next != null) {
                if (checkEquals(node, key)) {
                    node.value = value;
                    return;
                }
                node = node.next;
            }
            if (checkEquals(node, key)) {
                node.value = value;
            } else {
                node.next = newNode;
                size++;
            }
        }
        if ((double) size / hashMap.length >= GROW_INDEX) {
            grow();
        }
    }

    @Override
    public V getValue(K key) {
        int hash = getHash(key);
        int index = getIndex(hash);
        Node<K, V> node = hashMap[index];
        while (node != null) {
            if (checkEquals(node, key)) {
                return node.value;
            }
            node = node.next;
        }
        return null;
    }

    @Override
    public int getSize() {
        return size;
    }

    private int getHash(K key) {
        return Math.abs((key == null) ? 0 : key.hashCode());
    }

    private void grow() {
        Node<K, V>[] oldHashMap = hashMap;
        hashMap = new Node[hashMap.length * GROW_CONSTANT];
        size = 0;
        for (Node<K, V> current : oldHashMap) {
            while (current != null) {
                put(current.key, current.value);
                current = current.next;
            }
        }
    }

    private boolean checkEquals(Node<K, V> node, K key) {
        return node.key == key || key != null && key.equals(node.key);
    }

    private int getIndex(int hash) {
        return hash % hashMap.length;
    }

    private static class Node<K, V> {
        private int hash;
        private K key;
        private V value;
        private Node<K, V> next;

        private Node(int hash, K key, V value, Node<K, V> next) {
            this.hash = hash;
            this.key = key;
            this.value = value;
            this.next = next;
        }
    }
}
