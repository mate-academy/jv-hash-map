...
public class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    private int calculateIndex(K key) {
        int index = (key != null)? (key.hashCode() % currentCapacity) : 0;
        if (index < 0) {
            index = - index;
        }
        return index;
    ...
    private Node<K,V> putANewNode(int index, Node<K,V>[] tableToBeAddedTo, Node<K,V> nodeToPut) {
        ...
        if (tableToBeAddedTo[index] != null) {
            isSizeIncrease = passAcrossCurrentCapacityTableElementNodesToPutNewNode(tableToBeAddedTo,index,nodeToPut,isSizeIncrease);
        } else {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            while (transferredNode != null) {
                ...
                if (newTable[newTableIndex] != null) {
                    passAcrossExtendedTableElemElementNodesToPutTransferredNode(newTableIndex, newTable, transferredNode);
                } else {
                    newTable[newTableIndex] = new Node<>(transferredNode.key, transferredNode.value, null);
                }
    ...
    private void passAcrossExtendedTableElemElementNodesToPutTransferredNode(int newTableIndex, Node<K,V>[] newTable, Node<K,V> transferredNode) {
        ...
        while (currentNodeOfTableToBeAddedTo.next != null) {
            if (transferredNode.key == currentNodeOfTableToBeAddedTo.key
                    || transferredNode.key != null && transferredNode.key.equals(currentNodeOfTableToBeAddedTo.key)) {
        ...
        if (!areNodeKeysEqual) {
            currentNodeOfTableToBeAddedTo.next = new Node<K, V>(transferredNode.key, transferredNode.value, null);
        }
    }

    private boolean passAcrossCurrentCapacityTableElementNodesToPutNewNode(Node<K,V>[] tableToBeAddedTo, int index, Node<K,V> nodeToPut, boolean isSizeIncrease) {
        ...
        while (currentNodeOfTableToBeAddedTo != null) {
            if (nodeToPut.key == currentNodeOfTableToBeAddedTo.key
                    || nodeToPut.key != null && nodeToPut.key.equals(currentNodeOfTableToBeAddedTo.key)) {
        ...
        if (isSizeIncrease) {
            currentNodeOfTableToBeAddedTo.next = new Node<K, V>(nodeToPut.key, nodeToPut.value, null);
        }
